        ...
        <section className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          ...
          <form onSubmit={handleAddMetric} className="flex flex-col md:flex-row gap-3 mb-8">
            <input 
              ...
              className="flex-2 bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm outline-none focus:ring-2 focus:ring-[var(--color-primary)] font-bold" 
            />
            <input 
              type="text" 
              placeholder="kg, cm..." 
              ...
              className="flex-1 bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm outline-none focus:ring-2 focus:ring-[var(--color-primary)] font-bold text-center" 
            />
            <button 
              disabled={loading}
              className="bg-[var(--color-primary)] text-[var(--color-secondary)] px-8 py-4 rounded-2xl font-black uppercase text-xs shadow-lg hover:opacity-90 disabled:opacity-50"
            >
              {loading ? '...' : 'Agregar'}
            </button>
          </form>
            ...
            {metrics.map(m => (
              <div key={m.id} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-50 group">
                ...
                <button 
                  ...
                  className="text-[9px] font-black text-gray-300 hover:text-red-400 transition-colors uppercase tracking-widest"
                >
                  ELIMINAR
                </button>
              </div>
            ))}
            {metrics.length === 0 && <p className="text-center text-gray-300 font-bold py-4 text-xs uppercase tracking-widest">No hay ejercicios cargados</p>}
          </div>
        </section>
      </div>
    </div>
